...
def write_to_file(path, the_local_file, byte_count, repeat_write=False):
    ...
    if repeat_write:
        ...
        file_contents = cStringIO.StringIO()
        file_contents.write('a' * byte_count)
        rc[rc_id].fs.write_file(file_contents, id_=file_id)
...
def read_it(id, the_path, nfs=None):
    global rc
    f = cStringIO.StringIO()
    new_file_name = the_path.split("/")[-1:][0]
    dest_path = re.sub('/[^/]+$', '', the_path)
    dest_file = dest_path + "/" + new_file_name
    fullpath = nfs.get_fullpath(dest_file)
    nfs.read_file(fullpath, f)


def write_it(id, the_path, nfs=None):
    global rc

    file_contents = cStringIO.StringIO()
    file_contents.write('test ' * random.randint(300000,600000))
    ...
    fullpath = nfs.get_fullpath(dest_file)
    nfs.write_file(fullpath, file_contents)

    file_contents.close()
    file_contents = cStringIO.StringIO()
    file_contents.write('test ' * 10)
...
def load_it(heat, count, nfs=None, walterdelay=0.90, walter_filecount=30, walter_writesize=10000000, walter_delsize=200000, walter_processes=8):
    """defaults:
    walter_delay=0.90
    walter_filecount=10
    walter_writesize=7000000
    walter_delsize=20000
    walter_processes=8"""
    ...
    while tries < 15:
        try:
            res = rc[0].fs.get_file_samples('/', 20, 'capacity')  # ['capacity', 'capacity-growth', 'input-ops']
            top_files = {}
            for el in res:
            ...
            walterpool = Pool(processes=walter_processes)

            # debugging
            # print top_files_sorted
            # print file_count

            for i in range(0,count):
                file_reads = top_files_sorted[random.choice([random.randint(6, 10), 7]) if file_count > 10 else random.randint(0, file_count-1)]
                file_writes = top_files_sorted[random.choice([random.randint(3, 5), 7]) if file_count > 7 else random.randint(0, file_count-1)]
                ns_reads = top_files_sorted[random.choice([random.randint(2, 6), 7]) if file_count > 7 else random.randint(0, file_count-1)]
                ns_writes = top_files_sorted[random.choice([random.randint(4, 7), 7]) if file_count > 7 else random.randint(0, file_count-1)]

                if 'file_read' in heat:
                    result = pool.apply_async(read_it, args=(i%process_count, file_reads, nfs))
                if 'file_write' in heat and i % 150 == 0:
                    result = pool.apply_async(write_it, args=(i%process_count, file_writes, nfs ))
                if 'ns_read' in heat:
                ...
                if i == int(count * walterdelay):
                    for i in range(0, walter_filecount):
                        file_name = "important-file-" + str(i) + "." + random.choice(["mp3", "flac", "txt", "bmp", "jpg", "dat", "log"])
                        result = pool.apply_async(write_to_file, args=("/Share/walter/junk", file_name, walter_writesize))
                        resultwalter = walterpool.apply_async(write_to_file, args=("/Share/walter/junk", file_name, walter_writesize))
                        resultdel = delpool.apply_async(write_to_file, args=("/Share/walter/junk", file_name, walter_delsize))
            break

        except ValueError:  # we probably didn't see the files yet
            if tries == 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--walterdelay', help='Percentage of total ticks to delay Walter when using --heat')
    parser.add_argument('--nfsaddr', help='IP address of your NFS mount (assumes OS X standard /Volumes/NFSADDR')
